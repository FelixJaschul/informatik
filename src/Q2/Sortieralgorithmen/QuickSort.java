...
public class QuickSort implements Sort {
     ...
     * @param low Der Anfangsindex des aktuellen Teilarrays
     * @param high Der Endindex des aktuellen Teilarrays
    ...
    private int partition(int[] array, int low, int high) {
        ...
        for (int j = low; j < high; j++) {
            ...
            if (!SortierHilfe.compare(array[j], pivot)) { // Vergleich: array[j] <= pivot
                i += 1;
                SortierHilfe.swap(array, i, j);
            }
        }

        // Tausche das Pivot-Element an die richtige Position
        SortierHilfe.swap(array, i + 1, high);
